...
(defcustom neo-filepath-sort-function 'string<
  "Function to be called when sorting neotree nodes."  :type '(symbol (const :tag "Normal" string<)
  ...
  (let ((map (make-sparse-keymap)))
    ...
    (cond
     ((eq neo-keymap-style 'default)
      (define-key map (kbd "C-n") 'neotree-create-node)
      (define-key map (kbd "C-N") 'neotree-delete-node)
      (define-key map (kbd "C-r") 'neotree-rename-node)
      (define-key map (kbd "C-c") 'neotree-copy-node))
